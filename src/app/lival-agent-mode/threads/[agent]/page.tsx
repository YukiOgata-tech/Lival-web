        ...
        >
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-2 px-3 py-3 sm:gap-3 sm:px-6 sm:py-4">
            <div className="flex min-w-0 items-center gap-2 whitespace-nowrap sm:gap-3">
              ...
              <span className="ml-1 truncate text-xs text-gray-600 sm:ml-2 sm:text-sm font-medium">{initialFilter==='all'?'すべて':initialFilter} のスレッド一覧</span>
            </div>
          ...
          {createOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                >
                  {([
                    { key: 'tutor', label: 'Tutor', color: 'from-emerald-500 to-emerald-600', agent: 'tutor' as const, icon: Sparkles },
                    { key: 'planner', label: 'Planner', color: 'from-blue-500 to-blue-600', agent: 'planner' as const, icon: BookOpen },
                    { key: 'counselor', label: 'Counselor', color: 'from-purple-500 to-purple-600', agent: 'counselor' as const, icon: MessageSquare },
                  ]).map((item, idx) => {
                        ...
                        <motion.button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const local = JSON.parse(localStorage.getItem(key) || '[]') as Array<{ id: string; title: string; agent: string; updatedAt: number }>
                              const now = Date.now()
                              const next = [{ id, title: `${item.agent} スレッド`, agent: item.agent, updatedAt: now }, ...local]
                              localStorage.setItem(key, JSON.stringify(next))
                            } catch {}
                            if (item.agent === 'planner') router.push(`/lival-agent-mode/threads/planner/${id}`)
                            else if (item.agent === 'tutor') router.push(`/lival-agent-mode/threads/tutor/${id}`)
                            else router.push(`/lival-agent-mode/threads/counselor/${id}`)
                          }}
                          className={`bg-gradient-to-r ${item.color} w-full rounded-xl px-4 py-4 text-sm font-bold text-white shadow-lg hover:shadow-xl transition-all flex flex-col items-center gap-2`}
                        >
